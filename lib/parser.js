(function(){
	...
	var patterns={
		...
		src: /^\/([a-zA-Z0-9_-]+)\/src\/(.+)\.(js|css)$/,
		relative: /^\/src\/(.+)\.(js|css)$/,
		external: /^(http|https)?\:\/\/.+$/,
		inline: /^([.a-zA-Z0-9_-]*|\/)+\.(js|css)$/,
		pack: /^[\s|^.]*require[\s|^.]*\([\s|^.]*\[([\s|^.]*('|")[\s]*([.a-zA-Z0-9-_]|\/)+\.js[\s]*('|")[\s|^.]*(,)?[\s|^.]*)*[\s|^.]*\](.*|[\s|^.]*)*\)[\s|^.]*(;)?[\s|^.]*/,
	...
	var parsers=function(compiler){
		...
		$.extend(src.prototype,{
			parse: function(req){
				...
				if(request.mode=='pack'){
					...
					if(request.type=='js'){
						requires=compiler.getJSRequires(request.filepath);
					}else if(request.type=='css'){
						var requires=compiler.getCssRequires(request.filepath);
					}
					...
					if(request.type=='js'){
						requires.files.forEach(function(v){
							ret.push('document.write(\'<script type="text/javascript" src="'+request.protocol+'://'+request.headers.host+'/'+request.project+'/src/'+v+'?parent='+encodeURIComponent(request.file)+'"></script>\');');
						});
					}else if(request.type=='css'){
						requires.files.forEach(function(v){
		...
		$.extend(prd.prototype,{
			parse: function(req){
				...
				if(request.type=='js'){
					...
					code=btcss.minify(pack.dev.getCssSource(request));
					console.info('└──','[MINIFY FINISHED]'.green);
				}else{
					console.info('└──','[UNKNOWN URL]'.red);
				}
		...
		getJSRequires: function(p){
			...
			var ret=$.proxy(function(holepath){
				...
				if(patterns.pack.test(hole)){
					var script=vm.createScript(hole), children=[];
					script.runInNewContext({require: function(list,func){
						children=list;
					}});
		...
		getCssRequires: function(p){
			...
			var ret=$.proxy(function(holepath){
				...
				var file=this.readFile(holepath), dir=syspath.dirname(holepath);
				var hole=this.removeJSComments(file||''), _self=$.proxy(arguments.callee,this), _this=this, error=(file===false?[['#'.grey,'[WARN]'.yellow,ori,'not exist'.yellow].join(' ')]:[]);
