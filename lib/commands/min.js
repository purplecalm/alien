	...
	var min={
		...
		buildHtml: function(data,map,config){
			...
			if(data.css&&map[data.css]){
				css=map[data.css];
				console.info('├──','(include css)'.cyan,data.css.grey);
			}
			
			var js=false;
			if(data.js&&map[data.js]){
				js=map[data.js];
				console.info('├──','(include js)'.cyan,data.js.grey);
			}
			
			var body='<div style="position: absolute; background-color: #c00; color: #fff; font-size: 12px; right: 0; top: 0; height: 24px; line-height: 24px; padding: 0 10px; font-family: \'Microsoft Yahei\'">系统内容加载中...</div>';
			if(data.body){
				
				body=data.body;
				var path=syspath.join(root,'src',data.body);
			
				if(fs.existsSync(path)){
					console.info('├──','(include template)'.cyan,path.grey);
			...
			var code=[
				...
				'</head>\n<body>\n',
				body,
				js?'\n<script type="text/javascript" src="'+domain+'/'+config.job+'/prd/'+js+'"></script>\n':'',
				'</body></html>'];
			...
			console.info('└──','[BUILD FINISHED]'.green,'=>'.grey,path);
		},
		...
		one: function(path){
			...
			if(request.type=='js'){
				...
				fs.appendFileSync(minpath,code);
				fs.appendFileSync(verpath,ver);
				console.info('└──','[MINIFY FINISHED]'.green,'=>'.grey,minpath);
